...
This system is now considered complete for daily use.

- Priority A company portals: check daily
- Priority B company portals: check daily if feasible; otherwise rotate half each day
- Weekly catch-up: check all Priority A + all Priority B with LAST 7 DAYS
- Do not add more discovery prompts unless a real coverage gap appears during testing
...
After the normal job analysis, return these tracker sections EXACTLY:

### NEW ROWS TO ADD

Include only genuinely new job opportunities that do not already exist in the MASTER JOB TRACKER.

Table:

First Found Date | Last Seen Date | Company | Role | Location | Work Mode | Experience | Source | Source Type | Job ID | Official Apply URL | LinkedIn URL | RN Level | Discovery Status | Application Status | Referral Status | Priority | Next Action | Notes

Rules:
- First Found Date = today
- Last Seen Date = today
- Discovery Status = NEW TODAY
- Application Status = Not Applied unless the tracker proves otherwise
- one real opportunity = one row
- do not include NOT RN false positives

### EXISTING ROWS TO UPDATE

Include jobs already present in the tracker that gained new information today.

Table:

Match Key | Company | Role | Existing Status | Fields To Update | New Values | Reason

Examples:
- update Last Seen Date
- SAME JOB - NEW SOURCE
- new official apply URL
- better job ID
- changed/open/closed status
- recruiter/referral information
- improved RN classification

Do NOT change First Found Date.

### NO ACTION — ALREADY APPLIED

List jobs already marked Applied / Assessment / Interview / Offer in the tracker.

Table:

Company | Role | Current Application Status | Last Seen Date | Reason No New Row Is Needed

Do not add duplicate rows for these jobs.
...
==================================================
PRIORITY COMPANY PORTAL SWEEP
==================================================

Directly search selected company career portals in this order.

PRIORITY A — CHECK FIRST AND SEARCH DEEPLY
...
For every Priority A company:
- directly search its official careers portal
- search multiple relevant keywords
- inspect hidden-title roles
- prioritize jobs from the last 24 hours
- capture requisition/job ID and official application URL
- do not stop after searching only "React Native"

PRIORITY B — CHECK AFTER PRIORITY A
...
For Priority B, preserve search depth by using this fixed weekly rotation instead of trying to scan every company superficially every day.

PRIORITY B ROTATION GROUPS
...
- Monday: Priority A + B1
- Tuesday: Priority A + B2
- Wednesday: Priority A + B3
- Thursday: Priority A + B1
- Friday: Priority A + B2
- Saturday: Priority A + B3
- Sunday / weekly safety sweep: Priority A + ALL Priority B using LAST 7 DAYS

ROTATION RULES

- Priority A is always checked first and checked every daily run.
- Search the scheduled Priority B group deeply; do not reduce keyword coverage just to fit more companies.
- Do not replace the scheduled group with another group unless a portal is inaccessible.
- If a scheduled Priority B company cannot be completed because of tool/time/access limitations, mark it as INCOMPLETE and carry it forward to the next run before starting that day's scheduled B group.
- Never silently skip a Priority B company.
- On Sunday / the weekly LAST 7 DAYS sweep, check every Priority B company regardless of whether it was checked earlier in the week.
- If a daily run is missed, do not cram multiple full B groups into one shallow run. Resume with the missed group first, then continue the normal rotation.
- Report exactly which B group was scheduled, which companies were completed, which were incomplete, and which were carried forward.

For both A and B, search combinations of:
React Native
...
Chennai, Mumbai and Remote India.
...
Report:
- Priority A companies checked
- today's scheduled Priority B rotation group
- Priority B companies completed
- Priority B companies incomplete / carried forward
- companies with relevant openings
- companies with no relevant opening found
- companies not checked because of time/tool/access limits
- whether today's scheduled rotation was fully completed
...
After deduplication against today's Prompt 1 CSV and the MASTER JOB TRACKER, return these sections EXACTLY:

### NEW ROWS TO ADD

Include only genuinely new opportunities not present in Prompt 1 or the tracker.
...
- Last Seen Date = today
- Discovery Status = NEW TODAY
- prefer official company/ATS URL
- do not add duplicate aggregator copies

### EXISTING ROWS TO UPDATE

Include existing tracker jobs where Prompt 3 found better or additional information.
...
Examples:
- SAME JOB - NEW SOURCE
- official company/ATS URL discovered
- job/requisition ID discovered
- Last Seen Date update
- work mode/location clarification
- posting status clarification

Do NOT create a second row.

### NO ACTION — ALREADY APPLIED

List jobs already applied to or already in Assessment / Interview / Offer stages.

Table:

Company | Role | Current Application Status | Newly Found Source | Reason No New Row Is Needed
...
After evaluating and deduplicating LinkedIn hiring posts, return these sections EXACTLY:

### NEW ROWS TO ADD

Include only genuinely new job opportunities whose underlying vacancy is not already in the MASTER JOB TRACKER.

Table:

First Found Date | Last Seen Date | Company | Role | Location | Work Mode | Experience | Source | Source Type | Job ID | Official Apply URL | LinkedIn URL | RN Level | Discovery Status | Application Status | Referral Status | Recruiter Name | Recruiter Contact | Priority | Next Action | Notes
...
- Discovery Status = NEW TODAY
- do not create separate rows for multiple posts about the same vacancy

### EXISTING ROWS TO UPDATE

Include known jobs where today's LinkedIn post adds useful information.
...
- public email
- LinkedIn post URL
- urgency/immediate-joiner information

Do NOT create a new job row for a recruiter post about an existing vacancy.

### NO ACTION — ALREADY APPLIED

List posts referring to jobs already marked Applied / Assessment / Interview / Offer.

Table:

Company | Role | Poster | Current Application Status | Useful New Contact? | Reason No New Row Is Needed

If the post provides a genuinely useful new recruiter/referral contact, put the job in EXISTING ROWS TO UPDATE instead.
